...
const acceptanceTests: AcceptanceTest[] = [
  {
    ...
    run: async ({page}) => {
      await page.goto('/')
      await expect(page.getByRole('heading', {name: 'shell-werk'})).toBeVisible()
    }
  },
  {
  ...
  {
    title: 'Each test case (test / it block) maps 1:1 to a requirement acceptance criteria checkbox.',
  ...
  },
  {
    ...
    run: async ({page}) => {
      ...
      await expect(page.getByText(/models available/i)).toHaveText('1 models available')
      await page.getByRole('button', {name: 'Close'}).click()
    }
  },
  {
    title: 'CI uses mocked LLM providers; no real Ollama/vLLM servers are required.',
    run: async ({page}) => {
      await page.goto('/')
      const providerMode = await page.evaluate(() => {
        return (globalThis as {__E2E_PROVIDER_MODE?: string}).__E2E_PROVIDER_MODE
      })
  ...
  },
  {
    ...
    run: async ({page}) => {
      await page.goto('/')
      const expectedMode = process.env.E2E_USE_REAL_PROVIDER === '1' ? 'real' : 'mock'
      const providerMode = await page.evaluate(() => {
        return (globalThis as {__E2E_PROVIDER_MODE?: string}).__E2E_PROVIDER_MODE
      })
